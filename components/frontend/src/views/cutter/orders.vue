...
const table_option = ref({
  ...
  sortable: ["sku", "title", "material", "color", "size", "side"],
  sortIcon: {
    base: "sort-icon-none",
    up: "sort-icon-asc",
    down: "sort-icon-desc",
  ...
  customFilters: [
    {
      ...
      callback: (row, user) => {
        console.log(row);
        return row.product.sku.startsWith("INDI");
      },
    },
    {
      name: "all",
...
const mark_as_cut = (item) => {
  store
    .dispatch("orders/setOrderItemDone", {
      id: item,
      status: "cut",
    })
    .then(() => store.dispatch("order_items/fetchFilter", order_items_filter));
};

const delete_row = (item) => {
  if (
    confirm(
      `Уверены что хотите отправить ${selected_rows.value.length} изделий?`,
    )
  ) {
...
let clickedIndiItemsOnly = ref(false);
const indiItemsOnly = () => {
  clickedIndiItemsOnly.value = !clickedIndiItemsOnly.value;

  if (clickedIndiItemsOnly.value) {
    EventBus.emit("vue-tables.filter::indiOnly", store.state.auth.user.name);
  } else {
            ...
            <nav class="breadcrumb-one" aria-label="breadcrumb">
              <ol class="breadcrumb">
                <li class="breadcrumb-item">
                  <a href="javascript:;">Новые изделия</a>
                </li>
      ...
      <div class="col-xl-12 col-lg-12 col-sm-12 layout-spacing">
        <div class="panel br-6">
          <div class="custom-table">
            <v-client-table
              v-if="showTable"
              :data="store.state.order_items.order_items"
              :columns="columns"
            ...
            >
                  ...
                  >
                    {{
                      store.state.order_items.order_items.reduce(
                        (acc, e) =>
                          acc + (e.product.sku.startsWith("INDI") ? 1 : 0),
                        0,
                      )
                    }}</span
                  >
              ...
              <template #side="props">
                <div :data_sort="props.row.product.side">
                  {{ props.row.product.side }} -
                  {{ props.row.product.side_point }}
                </div>
              </template>
            </v-client-table>
          </div>
