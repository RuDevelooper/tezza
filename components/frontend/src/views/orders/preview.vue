...
let commentForSenderModal = null;
const commentForSenderModalRef = ref(null);
const initcommentForSenderModal = () => {
  commentForSenderModal = new Modal(commentForSenderModalRef.value);
};
...
const print = () => {
  let winPrint = window.open(
    `/docs/manager_order?id=${store.state.orders.order.id}&print=true`,
...
const add_track_number = () => {
  store
    .dispatch("orders/set_track_number", {
...
const update_comment_for_manager = () => {
  store
    .dispatch("orders/partial_update", {
      id: store.state.orders.order.id,
      payload: {
...
const update_comment_for_sender = () => {
  store
    .dispatch("orders/partial_update", {
      id: store.state.orders.order.id,
      payload: {
        comment_for_sender: store.state.orders.order.comment_for_sender,
      },
    })
    .then(() => {
      commentForSenderModal.hide();
      const toast = window.Swal.mixin({
        toast: true,
        position: "top-end",
        showConfirmButton: false,
                          ...
                          <div class="row">
                            ...
                            <div v-if="store.state.orders.order.customer" class="col-sm-8 align-self-center pt-5">
                              ...
                              <p class="inv-created-date">
                                ...
                                <span class="inv-date">
                                  {{
                                    store.state.orders.order.customer
                                      .organisation
                                  }}
                                </span>
                              </p>
                            ...
                            <div class="col-sm-4 align-self-start pt-5 text-sm-end">
                              ...
                              <p v-if="store.state.orders.order.assembling_start">
                                ...
                                <span>
                                  {{
                                    store.state.orders.order.assembling_start.toLocaleDateString(
                                      "ru-RU"
                                    )
                                  }}
                                </span>
                              ...
                              <p v-if="store.state.orders.order.shipped_at">
                                ...
                                <span>
                                  {{
                                    store.state.orders.order.shipped_at.toLocaleDateString(
                                      "ru-RU"
                                    )
                                  }}
                                </span>
